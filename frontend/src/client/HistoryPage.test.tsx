...
describe("HistoryPage summary rendering", () => {
  ...
  it("renders JSON object summary with readable labels and values", async () => {
    const summary = JSON.stringify({
      ...
      status: "finished",
      scenario_id: "generic_b2b_first_contact",
      raw_llm_payload: { prompt: "secret" },
      hidden_persona: { role: "secret" },
    });
    ...
    expect(screen.queryByText("raw_llm_payload")).not.toBeInTheDocument();
    expect(screen.queryByText("hidden_persona")).not.toBeInTheDocument();
