...
const COMPLEX_SUMMARY_FALLBACK = "Дополнительные данные доступны в отчёте.";
...
function formatParsedSummary(parsed: unknown, fallbackText: string): FormattedSessionSummary {
  ...
  if (Array.isArray(parsed)) {
    ...
    return values.length > 0 ? { kind: "items", items: [{ label: "Сводка", value: values }] } : { kind: "plain", text: COMPLEX_SUMMARY_FALLBACK };
  }
  if (isPlainObject(parsed)) {
    ...
    return items.length > 0 ? { kind: "items", items } : { kind: "plain", text: COMPLEX_SUMMARY_FALLBACK };
  }
