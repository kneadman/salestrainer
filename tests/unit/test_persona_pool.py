...
from app.application.persona_pool_service import (
    PersonaPoolService,
    build_pool_context_hash,
    pool_prompt_version,
)
...
def test_pool_prompt_version_tracks_prompt_content() -> None:
    """A persona-prompt edit must invalidate the reserve.

    The durable history column keeps the stable contract label. The pool key must
    instead use the content revision, or sessions would keep receiving personas
    generated by the previous prompt text.
    """
    from app.domain.contract_versions import PERSONA_PROMPT_VERSION
    from app.prompts.versions import prompt_revisions

    assert pool_prompt_version() == prompt_revisions()["persona"]
    assert pool_prompt_version() != PERSONA_PROMPT_VERSION
    assert prompt_revisions()["persona"].startswith("persona_generator-")


def test_stale_prompt_revision_reserve_is_not_served() -> None:
    """Rows written under a different prompt revision are invisible and dropped."""
    session = _create_session()
    config = _persist_config(session)
    ...
    repository.add_entry(
        ...
        context_hash=build_pool_context_hash(config),
        persona=valid_minimal_persona().model_dump(mode="json"),
        persona_prompt_version="persona_generator-outdated0000",
    )
    service = _service(session, _StubGenerator())
    ...
    # The outdated row was cleaned up rather than left to accumulate.
    versions = {row.persona_prompt_version for row in session.query(PersonaPoolEntry).all()}
    assert versions == {pool_prompt_version()}
